...
const sendVerificationEmail = async (email, code) => {
  ...
  const from = process.env.SMTP_FROM || '"CipherNest" <no-reply@ciphernest.com>';
  ...
  try {
    ...
    const mailOptions = {
      from,
      to: email,
      subject: 'CipherNest — Email Verification Code',
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 30px; border: 1px solid #e2e8f0; border-radius: 8px; color: #0f172a;">
          <h2 style="text-align: center; color: #000; font-size: 24px; margin-bottom: 24px; letter-spacing: -0.5px;">CipherNest</h2>
          ...
          <p style="color: #94a3b8; font-size: 11px; text-align: center; margin: 0;">This is an automated security transmission from CipherNest.</p>
        </div>
      `,
    };
